...
find "${derivatives}/data" -type d -name 'sub-*' | sort -V | while read -r dir; do
	...
	xfm="${derivatives}/data/${sub}/xfms"
 	...
 	# define derivative dwi directory
  	dwi="${derivatives}/data/${sub}/dwi"

   	# define derivative anat directory
    	anat="${derivatives}/data/${sub}/anat"
	...
	dwiextract \
	"${dwi}/eddy/${sub}_dwi_edc.nii.gz" - -bzero \
	-fslgrad \
	"${dwi}/eddy/${sub}_dwi_edc.eddy_rotated_bvecs" \
	"${rawdata}/${sub}/dwi/${sub}_dwi.bval" \
	| mrmath - mean "${dwi}/eddy/${sub}_dwi_edc_b0.nii.gz" \
	-axis 3

	# Brain extract the edc b0
	bet \
	"${dwi}/eddy/${sub}_dwi_edc_b0.nii.gz" \
	"${dwi}/eddy/${sub}_dwi_edc_b0_brain.nii.gz"
